...
function AudioGeneration() {
  ...
  const fetchAudio = async () => {
    ...
    setLoading(true);
    try {
      const response = await axiosIstance.post("/models/audio-generation", { prompt: input },
        {responseType: 'blob'});
  ...
  const saveAudio = () => {
    ...
    link.href = audioUrl;
    link.download = 'generated-audio.mp3'; // Default filename
  ...
  return (
    <div className=' flex flex-col mx-10 items-center mt-4'>
        <h1 className=' text-center text-4xl my-10'>Audio Generation</h1>
